...
void print_dmic_nhlt_bytes(struct dmic_configuration *cfg)
{
	...
	src = (uint8_t *)&dmic_nhlt;
	dst = (char *)&line[0];

	tplg_pp_debug("C version:");
	tplg_pp_debug("const uint8_t nhlt_dmic_cfg[%d] = {", length);
	...
	tplg_pp_debug("Alsa topology version:");
	tplg_pp_debug("bytes \"");
			 ...
			 snd_config_t *dai_cfg)
{
	...
	tplg_pp_debug("dai_index: %d", dai_index);
	tplg_pp_debug("num_pdm_active: %d", num_pdm_active);
	tplg_pp_debug("fifo_word_length: %d", fifo_word_length);
	tplg_pp_debug("clk_min: %d", clk_min);
	tplg_pp_debug("clk_max: %d", clk_max);
	tplg_pp_debug("duty_min: %d", duty_min);
	tplg_pp_debug("duty_max: %d", duty_max);
	tplg_pp_debug("sample_rate: %d", sample_rate);
	tplg_pp_debug("unmute_ramp_time: %d", unmute_ramp_time_ms);

	dai_index = dai_index_t;
	dmic_prm[dai_index].num_pdm_active = num_pdm_active;
	dmic_prm[dai_index].fifo_bits = fifo_word_length;
	dmic_prm[dai_index].pdmclk_min = clk_min;
	dmic_prm[dai_index].pdmclk_max = clk_max;
	dmic_prm[dai_index].duty_min = duty_min;
	dmic_prm[dai_index].duty_max = duty_max;
	dmic_prm[dai_index].fifo_fs = sample_rate;
	dmic_prm[dai_index].unmute_ramp_time = unmute_ramp_time_ms;
			...
			snd_config_t *cfg)
{
	...
	const char *id;
	long int_val = 0;
	int ctrl_id, mic_a_enable, mic_b_enable, polarity_a, polarity_b, clk_edge, skew;
	int ret;
	snd_config_t *class_cfg;

	tplg_pp_debug("set_pdm_data");
	...
	tplg_pp_debug("pdm ctrl_id: %ld", ctrl_id);
	tplg_pp_debug("pdm mic_a_enable: %ld", mic_a_enable);
	tplg_pp_debug("pdm mic_b_enable: %ld", mic_b_enable);
	tplg_pp_debug("pdm polarity_a: %ld", polarity_a);
	tplg_pp_debug("pdm polarity_b: %ld", polarity_b);
	tplg_pp_debug("pdm clk_edge: %ld", clk_edge);
	tplg_pp_debug("pdm skew: %ld", skew);

	pdm_index = ctrl_id;
	dmic_prm[dai_index].pdm[pdm_index].enable_mic_a = mic_a_enable;
	dmic_prm[dai_index].pdm[pdm_index].enable_mic_b = mic_b_enable;
	dmic_prm[dai_index].pdm[pdm_index].polarity_mic_a = polarity_a;
	dmic_prm[dai_index].pdm[pdm_index].polarity_mic_b = polarity_b;
	dmic_prm[dai_index].pdm[pdm_index].clk_edge = clk_edge;
	dmic_prm[dai_index].pdm[pdm_index].skew = skew;

	return 0;
}

int set_dmic_nhlt(struct tplg_pre_processor *tplg_pp,
		  snd_config_t *cfg)
{
	snd_config_t *items;
	int ret;
	...
	tplg_pp_debug("set_dmic_nhlt");

	set_dmic_data(tplg_pp, cfg);
	...
	snd_config_for_each(i, next, items) {
		...
		tplg_pp_debug("set_pdm_data for  id: '%s' ...", id);

		set_pdm_data(tplg_pp, n);
	}
